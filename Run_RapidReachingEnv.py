...
def generate_random_points_3d(num_points: int,
    ...
    y_points = np.random.uniform(y_range[0], y_range[1], num_points)
    z_points = np.random.uniform(z_range[0], z_range[1], num_points)
...
if __name__ == '__main__':
    ...
    # examples of singularity: 109, 150
    ...
    original_model = os.path.join(dir_path, "./Robot/miniArm.xml")
    modified_model = os.path.join(dir_path, "./Robot/miniArm_with_points.xml")

    # Add points to the model
    add_points_to_xml(original_model, points, modified_model)

    # Initialize the viewer and start the simulation
    launch_from_path(modified_model)
